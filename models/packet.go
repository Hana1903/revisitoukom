...
func (p *Packet) UnmarshalJSON (data []byte) error  {
	...
	//Proses Unmarshalling JSON
	if err := json.Unmarshal(data, &tmp); err != nil {
		return err
	}

	//Memindahkan data struct sementara (tmp) ke struct utama (p)
	p.ID = tmp.ID
	...
	//Menangani tipe data dinamis (string atau float64) secara fleksibel dan aman
